...
def test_configureFirewall_IPTables_enabled_no_errors(Pihole):
    ...
    assert 'iptables -C INPUT -p tcp -m tcp --dport 80 -j ACCEPT' in firewall_calls
    assert 'iptables -C INPUT -p tcp -m tcp --dport 53 -j ACCEPT' in firewall_calls
    assert 'iptables -C INPUT -p udp -m udp --dport 53 -j ACCEPT' in firewall_calls
