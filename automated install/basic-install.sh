...
package_manager_detect() {
  ...
  # If neither apt-get or yum/dnf package managers were found
    ...
    INSTALLER_DEPS=(git iproute2 dialog ca-certificates musl-locales ncurses libcap coreutils)
...
create_pihole_user() {
  ...
  if id -u pihole &> /dev/null; then
    ...
    if getent group pihole > /dev/null 2>&1; then
      ...
      if command -v groupadd &> /dev/null && groupadd pihole; then
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
      ...
      elif command -v addgroup &> /dev/null && addgroup pihole; then
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
        local str="Adding user 'pihole' to group 'pihole'"
        printf "  %b %s..." "${INFO}" "${str}"
        # if pihole user can be added to group pihole
    ...
    if getent group pihole > /dev/null 2>&1; then
      ...
      if command -v useradd &> /dev/null && useradd -r --no-user-group -g pihole -s /usr/sbin/nologin pihole; then
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
      elif command -v adduser &> /dev/null && adduser -SDG pihole pihole; then
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
      else
        printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
      fi
      ...
      if command -v useradd &> /dev/null && useradd -r -s /usr/sbin/nologin pihole; then
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
      elif command -v addgroup &> /dev/null && addgroup pihole; then
        adduser -SDG pihole pihole
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
      else
        printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
      fi
...
main() {
  ...
  if [[ "${useUpdateVars}" == false ]]; then
    ...
    if [[ -f "${PI_HOLE_CONFIG_DIR}/pihole-FTL.conf" ]]; then
      PRIVACY_LEVEL=$(sed --quiet --expression='s/PRIVACYLEVEL=\(.*\)/\1/p' "${PI_HOLE_CONFIG_DIR}/pihole-FTL.conf")
