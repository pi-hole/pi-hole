...
checkout() {
  ...
  if [[ "${1}" == "dev" ]] ; then
    ...
    read -ra corebranches < <( get_available_branches "${PI_HOLE_FILES_DIR}" )
    ...
    read -ra webbranches < <( get_available_branches "${webInterfaceDir}" )
    ...
    if check_download_exists "$path"; then
      ...
      read -ra ftlbranches < <( git ls-remote https://github.com/pi-hole/ftl | grep 'heads' | sed 's/refs\/heads\///;s/ //g' | awk '{print $2}' )
