...
def main() -> None:
    ...
    mcp_parser.add_argument("--port", type=int, default=8741, help="Port for HTTP transport")
    mcp_parser.add_argument("--host", default="127.0.0.1", help="Bind address for HTTP transport")
    mcp_parser.add_argument(
    ...
    analyze_parser = subparsers.add_parser("analyze", help="One-shot DNS traffic analysis")
    analyze_parser.add_argument("--hours", type=int, default=24, help="Hours of data to analyze")

    # ── pihole agent chat ──
    subparsers.add_parser("chat", help="Interactive conversation with the agent")

    # ── pihole agent monitor ──
    monitor_parser = subparsers.add_parser("monitor", help="Real-time traffic monitoring")
    monitor_parser.add_argument("--daemon", action="store_true", help="Run as background daemon")
    monitor_parser.add_argument("--stop", action="store_true", help="Stop monitoring daemon")
    monitor_parser.add_argument("--interval", type=int, default=60, help="Poll interval in seconds")

    # ── pihole agent status ──
    subparsers.add_parser("status", help="Show agent configuration and status")

    # ── pihole agent rollback ──
    rollback_parser = subparsers.add_parser("rollback", help="Undo recent agent blocking actions")
    rollback_parser.add_argument("n", nargs="?", type=int, default=1, help="Number of actions to undo")

    # ── pihole agent config ──
    config_parser = subparsers.add_parser("config", help="View/set agent configuration")
    config_parser.add_argument("key", nargs="?", help="Configuration key (e.g., safety.blocking_mode)")
    ...
    log_parser.add_argument("--tail", type=int, default=20, help="Number of recent entries")
    ...
    if args.command == "mcp":
        ...
        print(f"The '{args.command}' command will be available after Phase {_phase_for(args.command)} implementation.")
...
def _handle_status() -> None:
    ...
    print(f"  Rate Limit:       {config.safety.rate_limit_per_minute}/min, {config.safety.rate_limit_per_hour}/hr")
...
def _handle_config(args: argparse.Namespace) -> None:
    ...
    if args.key is None:
        ...
        print(json.dumps({
            ...
            "mcp": {"transport": config.mcp.transport, "port": config.mcp.port, "host": config.mcp.host},
